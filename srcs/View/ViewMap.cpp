...
void ViewMap::updateUp(BlockMap *block_map, std::vector<ViewChunk *> *queueChunk)
{
    stop.lock();
    for (size_t i = 0; i <= RENDER_DISTANCE * 2; i++)
        queueChunk->push_back(this->_map[RENDER_DISTANCE * 2][i]);
    stop.unlock();
...
void ViewMap::updateDown(BlockMap *block_map, std::vector<ViewChunk *> *queueChunk)
{
    stop.lock();
    for (size_t i = 0; i <= RENDER_DISTANCE * 2; i++)
        queueChunk->push_back(this->_map[0][i]);
    stop.unlock();
    for (size_t i = 0; i < RENDER_DISTANCE * 2; i++)
    {
        for (size_t j = 0; j <= RENDER_DISTANCE * 2; j++)
...
void ViewMap::updateLeft(BlockMap *block_map, std::vector<ViewChunk *> *queueChunk)
{
    stop.lock();
    for (size_t i = 0; i <= RENDER_DISTANCE * 2; i++)
        queueChunk->push_back(this->_map[i][RENDER_DISTANCE * 2]);
    stop.unlock();
    for (size_t i = RENDER_DISTANCE * 2; i > 0; i--)
    {
        for (size_t j = 0; j <= RENDER_DISTANCE * 2; j++)
...
void ViewMap::updateRight(BlockMap *block_map, std::vector<ViewChunk *> *queueChunk)
{
    stop.lock();
    for (size_t i = 0; i <= RENDER_DISTANCE * 2; i++)
        queueChunk->push_back(this->_map[i][0]);
    stop.unlock();
    for (size_t i = 0; i < RENDER_DISTANCE * 2; i++)
    {
        for (size_t j = 0; j <= RENDER_DISTANCE * 2; j++)
