...
{
    this->_layers.push_back(BlockLayer(1, SEA_LEVEL - 1, Stone(), Stone(), Stone(), 0));
    this->_layers.push_back(BlockLayer(SEA_LEVEL, 255, Stone(), Dirt(), Grass(), 4));
}

TestDesert::TestDesert(void) : Biome("test", 1, 255, plain_curve)
{
    this->_layers.push_back(BlockLayer(1, SEA_LEVEL - 1, Stone(), Stone(), Stone(), 0));
    this->_layers.push_back(BlockLayer(SEA_LEVEL, 255, Stone(), Sand(), Sand(), 7));
}

TestSnow::TestSnow(void) : Biome("test", 1, 255, plain_curve)
{
    this->_layers.push_back(BlockLayer(1, SEA_LEVEL - 1, Stone(), Stone(), Stone(), 0));
    this->_layers.push_back(BlockLayer(SEA_LEVEL, 255, Stone(), Dirt(), SnowyGrass(), 4));
}
